...
def get_tsne_df(predictions, embd_columns, hue_column = None):
    ...
    embd_columns -> column name for the embedding column as str  
    ...
    predictions['np_array'] = predictions[embd_columns].apply(lambda x: np.array(x))
...
def show(session_state):
    ...
    model_names = st.sidebar.multiselect(
            ...
            ,NLU_MODEL_NAMES
            ,session_state.sent_embed_selected_model_names.split() # Remember selection
            )
    ...
    session_state.sent_embed_input_format = st.sidebar.radio(
            ...
            ,INPUT_FORMATS
            ,index=int(np.where(np.array(INPUT_FORMATS) == session_state.sent_embed_input_format)[0][0]) # Remember selection
    )
    ...
    if session_state.sent_embed_selected_model_names or session_state.sent_embed_loaded_model_names:
        ...
        if btn_load and (session_state.sent_embed_selected_model_names == session_state.sent_embed_loaded_model_names):
            with st.spinner("**Info**: Selected models '{}' already loaded. Stop request.".format(session_state.sent_embed_selected_model_names)):
                time.sleep(3)
            btn_load = False
        # Case: load selected model
        if btn_load:
            with st.spinner("Download started this may take some minutes ... :coffee:"):
                session_state.sent_embed_pipe = nlu.load(session_state.sent_embed_selected_model_names)
                session_state.sent_embed_loaded_model_names = ' '.join(model_names)
        ...
        if session_state.sent_embed_loaded_model_names: 
            st.success("**Info**: loaded models are: {} ".format(session_state.sent_embed_loaded_model_names))
            ...
            if uploaded_file and len(session_state.sent_embed_csv_input) == 0:
                session_state.sent_embed_csv_input = pd.read_csv(uploaded_file, sep=",", header=[0], encoding="utf-8", dtype = "unicode")
            ...
            if len(session_state.sent_embed_csv_input) > 0:
                ...
                # Map Column
                st.write('**Map Column**')
                COLUMNS_NAMES = ["-"] + session_state.sent_embed_csv_input.columns.tolist()
                session_state.sent_embed_csv_txt_column_name = st.selectbox("Select text column"
                                                                            ,COLUMNS_NAMES
                                                                            ,index = int(np.where(np.array(COLUMNS_NAMES) == session_state.sent_embed_csv_txt_column_name)[0][0]) # Remember selection
                                                                            )
                    ...
                    if btn_pred:
                        ...
                        with st.spinner("Calculation started ... :coffee:"):
                            session_state.sent_embed_csv_out = session_state.sent_embed_pipe.predict(
                                ...
                                ,output_level = session_state.sent_embed_input_format
                                ,positions=True)
                ...
                if session_state.sent_embed_csv_is_predicted:
                    ...
                    link = get_table_download_link(session_state.sent_embed_csv_out)
